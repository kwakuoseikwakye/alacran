 ...
 *
 * First backup: `gh repo create` with `--source`/`--push` does create + wire up
 * `origin` + push in one step. ALWAYS `--private` — a company repo carries real
 * business context (ontology, decisions, notes), so a public default would be a
 * privacy incident, not a preference.
 *
 * Subsequent backups: just push to the remote that already exists.
 */
